...
export const backupManifestSchema = z.object({
  ...
  counts: z.object({
    ...
    forecastRules: z.number().int().nonnegative(),
    forecastOccurrences: z.number().int().nonnegative(),
    emergencyFundConfigurations: z.number().int().nonnegative(),
    emergencyFundAccounts: z.number().int().nonnegative(),
    plaidUsers: z.number().int().nonnegative(),
    plaidItems: z.number().int().nonnegative(),
...
export function migrationFingerprint() {
  ...
  const migrationFiles = [
    ...
    "20260712230000_debt_payoff_plan/migration.sql",
    "20260712233000_decision_scenarios/migration.sql",
