...
import {
  ...
  Pill,
  PlannedControl,
} from "@/components/data-display/primitives";
  ...
  return (
    <AppShell>
      ...
      <div className="mb-4 flex flex-wrap gap-3">
        <input
          className="h-11 min-w-[280px] flex-1 rounded-md border border-[var(--border)] bg-white px-4"
          placeholder="Search merchant..."
        />
        {["All Accounts", "All Categories", "All Statuses"].map((label) => (
          <select
            key={label}
            className="h-11 rounded-md border border-[var(--border)] bg-white px-4"
          >
            <option>{label}</option>
          </select>
        ))}
          ...
          <table className="w-full min-w-[980px] text-left text-sm">
            ...
            <tbody>
              {transactions.map((row, index) => (
                <tr
                  key={`${row[0]}-${row[1]}-${row[2]}-${index}`}
                  className="border-b border-[var(--border)] hover:bg-[var(--surface-muted)]"
                >
                  <td className="px-4 py-3">
                    <span className="block h-5 w-5 rounded border border-[var(--teal)]" />
                  </td>
                  ...
                  <td className="px-4 py-3">
                    {row[4] === "Uncategorized" ? (
                      <span className="italic text-[var(--amber)]">{row[4]}</span>
                    ) : (
                      row[4]
                    )}
                  </td>
      ...
      {drawer ? (
        <div className="fixed inset-0 z-50 bg-black/70" onClick={() => setDrawer(false)}>
          <aside
            ...
            className="ml-auto h-full w-full max-w-[520px] overflow-y-auto bg-[var(--surface)] p-8"
          ...
          >
            ...
            <p className="mt-4 rounded-md border border-[var(--amber)] bg-[var(--amber-soft)] p-3 text-sm text-[var(--muted)]">
              Demonstration-only editor: normalized values can be changed on screen, but transaction
              persistence is not implemented in Phase 1.
            </p>
            <div className="my-8 flex justify-between border-b border-[var(--border)] pb-6">
              <span className="text-[var(--muted)]">Amount</span>
              <strong className="text-3xl">-$87.42</strong>
            </div>
            <Section
              title="Imported Data"
              rows={[
                ["Raw merchant", "WHOLE FOODS MARKET #123"],
                ["Raw amount", "-87.42"],
                ["Imported on", "Jul 8, 2026"],
              ]}
            />
            <div className="mt-8 space-y-4">
              ...
              {["Merchant", "Category", "Transaction type", "Notes"].map((label, index) => (
                <label key={label} className="block text-sm text-[var(--muted)]">
                  {label}
                  <input
                    className="mt-2 h-10 w-full rounded-md border border-[var(--border)] px-3 text-[var(--text)]"
                    defaultValue={["Whole Foods Market", "Groceries", "Debit", ""][index]}
            ...
            <div className="mt-8 grid grid-cols-2 gap-3">
              {["Match Transfer", "Split", "Exclude", "Merchant Rule"].map((label) => (
                <Button
                  key={label}
                  variant="secondary"
                  disabled
                  title={`${label} is planned for a later phase`}
                >
                  {label}
                </Button>
              ))}
  ...
  return (
    <div>
      ...
      <div className="space-y-3">
        {rows.map(([a, b]) => (
          <div key={a} className="flex justify-between gap-8">
